...
public class FlyCheck extends AbstractCheck {
    ...
    private static boolean hasSupport(ServerPlayer player) {
        // The server validated the collision when it set onGround; the support
        // probe below covers partial block edges and rideable entity surfaces.
        if (player.onGround()) return true;
        return hasSupportAt(player, player.getX(), player.getY(), player.getZ());
    }
    ...
    private static boolean hasSupportAt(ServerPlayer player, double x, double y, double z) {
        ...
        if (isSolid(player, feet) || isSolid(player, feet.below())) return true;
